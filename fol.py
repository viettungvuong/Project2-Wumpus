        ...
        if newsentence[i] == "," or newsentence[i] == "(":
            if newsentence[i + 1] not in capitalVariables:
                ...
                if not substitution:
                    ...
                    newsentence = (
                        newsentence[: i + 1] + variableArray[number] + newsentence[j:]
                    )
...
pattern = re.compile(
    "\||&|=>"
)  # we can remove the '\|'' to speed up as 'OR' doesnt come in the KB
pattern1 = re.compile("[(,]")
for i in range(0, k):
    ...
    for j in range(0, lenoftemp):
        ...
        if predicate[0] in kb:
            if lengthofpredicate in kb[predicate[0]]:
                kb[predicate[0]][lengthofpredicate].append(
                    [kbbefore[i], temp, j, predicate[1:]]
                )
            else:
                kb[predicate[0]][lengthofpredicate] = [
                    kbbefore[i],
                    temp,
                    j,
                    predicate[1:],
            ...
            kb[predicate[0]] = {
                lengthofpredicate: [[kbbefore[i], temp, j, predicate[1:]]]
            }
...
def unificiation(arglist1, arglist2):
    ...
    for i in range(len(arglist1)):
        if (
            arglist1[i] != arglist2[i]
            and (arglist1[i][0] in capitalVariables)
        ...
        elif (arglist1[i][0] in capitalVariables) and not (
        ...
        elif not (arglist1[i][0] in capitalVariables) and (
            arglist2[i][0] in capitalVariables
        ):
            if arglist1[i] not in theta.keys():
                theta[arglist1[i]] = arglist2[i]
                arglist1 = substituevalue(arglist1, arglist1[i], arglist2[i])
        elif not (arglist1[i][0] in capitalVariables) and not (
            arglist2[i][0] in capitalVariables
        ):
            if arglist1[i] not in theta.keys():
                theta[arglist1[i]] = arglist2[i]
            ...
            for ac in range(0, lenansclauses):
                ...
                if not x:
                    ...
                    for numofpred in range(0, lenofx):
                        ...
                        thetalist = unificiation(
                            copy.deepcopy(sentenceselected[3]),
                            copy.deepcopy(ansclausespredicate[1:]),
                        )
                        if len(thetalist) != 0:
                            ...
                            for remain in range(0, len(ansclauseleft)):
                                ...
                                ansclauseleft[remain] = replace(
                                    listansclauseleft, thetalist[2]
                                )
